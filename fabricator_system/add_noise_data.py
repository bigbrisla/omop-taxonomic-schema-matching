...
def clean_word(word: str):
    get_vals = list([val for val in word if val.isalnum()])
...
def sub_job(args: tuple):
    ...
    # Text columns: pandas <2.0 reports 'object', pandas >=2.x may report
    # 'str'/'string' for the dedicated string dtype. Accept all of them so the
    # typo-noise path is not silently skipped on modern pandas.
    text_dtypes = ('object', 'str', 'string')
    if type_col == np.dtype('float64'):
        np_frame = np.array(frame)
        new_frame = perturb_data(np_frame)
        return col, new_frame
    elif type_col not in text_dtypes:
        return col, frame
    else:
        for rec in range(len(frame)):
            ...
            if pd.isnull([frame[rec]]):
                ...
                for i in ltc:
                    pick = proximity_typo(word[i].lower())
                    while word[i] == pick:
...
def update_values(frame: pd.DataFrame, approx_prc: int):
    list_tups = []
