...
def split(source: Dataset, overlap: float, rand: bool, approx: bool, approx_prc: int = 0):
    ...
    if rand:
        ...
        target1_data = pd.concat([target1_data, tmp])
    else:
...
def overlap2sources(source1: Dataset, source2: Dataset, overlap: float, rand: bool):
    ...
    if rand:
        ...
        target2_data = pd.concat([target2_data, source2.data.iloc[commons]])

    else:
        target2_data = pd.concat([target2_data, source2.data.iloc[:int(overlap * half)]])
